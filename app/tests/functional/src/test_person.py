...
async def test_get_person_all_cache_request(data_dict: dict, es_write_data):
    await es_write_data(data_type="persons")
...
async def test_get_person_all_by_search_cache_request(data_dict: dict, es_write_data):
    await es_write_data(data_type="persons")
    await TestApiBase(**data_dict).perform(from_cache=True)
...
async def test_get_person_by_id_cache_request(data_dict: dict, es_write_data):
    await es_write_data(data_type="persons")
    await TestApiBase(**data_dict).perform(from_cache=True)
